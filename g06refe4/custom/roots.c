...
void load_volume_table() {
    ...
    // Insert an entry for /, which is the ramdisk and is always mounted.
    device_volumes[1].mount_point = "/";
...
int smart_device_mounted(Volume *vol) {
    ...
    if (vol->device != NULL) {
        tmp = strchr(vol->device, '#');
        len = tmp - vol->device;
        if (tmp && len < 255) {
            strncpy(device_name, vol->device, len);
